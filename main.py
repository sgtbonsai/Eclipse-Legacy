...
#slash commands are a social construct anyways
...
async def invite(ctx):
    embed = discord.Embed(
        title="Invite Me!",
        description="Thank you for considering me for a new server!",
        color=discord.Color.orange(),
        icon_url=
        "https://cdn.discordapp.com/avatars/803862179546660934/bd4c3105280b890b53b38f6812ab45b0.png?size=1024"
    )
    embed.add_field(
        name="Thank You for supporting me!",
        value=
        '[Invte Eclipse Today!](https://discord.com/api/oauth2/authorize?client_id=803862179546660934&permissions=8&scope=bot)',
        inline=True)
...
async def test(ctx):
    embed = discord.Embed(title="The test is successful!",
                          description='For help, please do "e!help"',
                          color=discord.Color.orange())
    await ctx.send(embed=embed)
...
async def say(ctx, *, args, amount=1):
    ...
    embed = discord.Embed(title=f"{ctx.author.mention} made me say something!",
                          description=" ",
                          color=discord.Color.orange())
    embed.add_field(name=f'"{args}"',
                    value="Look, they said it, not me.",
                    inline=True)
    await ctx.send(embed=embed)
...
@client.command()
@commands.is_owner()
async def sayy(ctx, *, args, amount=1):
...
async def RedCup(ctx):
    await ctx.send(
        "this is a red cup lmao i'm tired and the voices are getting closer and closer every day please help I can't take it anymore pleas HELP!!!!"
    )


@client.command()
async def AppleCloth(ctx):
    AppleCloth = discord.Embed(
        title="Apple Cloth",
        description=
        ...
        color=discord.Color.orange())
    AppleCloth.set_thumbnail(
        url=
        "https://cdn.discordapp.com/attachments/805878097756684288/902322733097058344/F16E07ED-7249-4FE7-AE3D-B4F5B47D0CA7.png"
    )
    AppleCloth.add_field(
        name="Get your Apple Cloth for only $20",
        value=
        '[Get your Apple Cloth here!!](https://www.apple.com/shop/product/MM6F3AM/A/polishing-cloth)',
        inline=True)
...
async def ping(ctx):
    embed = discord.Embed(colour=discord.Color.orange(),
                          ...
                          description=" ")
    embed.add_field(
        name=f"My ping is {round(client.latency * 1000)}ms",
        value=
        "the irs is at my doorstep asking me why i didn't pay my taxes in 2007...",
        inline=False)
    await ctx.send(embed=embed)
...
async def pong(ctx):
    await ctx.send(
        'gruiehijrgejoirgwjejgorihrwgi∆˜ˆ∫˙¥¨√†ç®§≈¶ç†¨¥√¨∫˙ˆ∆ø˚πøˆ∆˙¨©¥†®ƒ¥∂´ƒ†©¥ø˙¨øˆ you have killed me with your bad speeeling'
    )
...
async def on_command_completion(ctx, *, args):
    ...
    log = discord.Embed(colour=discord.Colour.green(),
                        title="Command Executed!")
    log.add_field(name="Command", value=f"`{args}`", inline=False)
    log.add_field(name="User:", value=f"{ctx.author.mention}", inline=False)
    log.add_field(name="Channel:",
...
async def mute(ctx, user: discord.Member, duration=0, *, unit=None):
    roleobject = discord.utils.get(ctx.message.guild.roles,
                                   id=847573758695833671)
...
async def help(ctx):
    ...
    embed.add_field(
        name="Say",
        value=
        "Says the thing you want me to say\n Usage: \n`b!say <message here>`",
        inline=True)
    embed.add_field(
        name="Userinfo",
        value=
        "Responds with the info of the stated user\n Usage: \n`b!userinfo <id>`",
        inline=True)
    embed.add_field(
        name="Add",
    ...
    embed.add_field(
        name="Subtract",
        value=
        "Subtracts the stated numbers\n Usage: \n`b!subtract <number number>`",
        inline=True)
    embed.add_field(
        name="Multiply",
        value=
        "Multiplies the stated numbers\n Usage: \n`b!multiply <number number>`",
        inline=True)
    embed.add_field(
        name="Divide",
        value=
        "Divides the stated numbers\n Usage: \n`b!divide <number number>`",
        inline=True)
    embed.add_field(
        name="Ban",
    ...
    embed.add_field(
        name="Messagecount",
        value=
        "Counts the messages in the stated channel\n Usage: \n`b!messagecount <channel>`",
        inline=True)
    embed.add_field(
        name="Invite",
    ...
    embed.add_field(
        name="RPS",
        value=
        "Play 'rock paper scissors' with Eclipse!/n Usage: /n`b!rps <rock, paper, or scissors>`",
        inline=True)
    embed.add_field(
        name="Mock",
        value=
        "mAkE eCliPse mOck A qUoTe/n Usage: /n `b!mock <message/quote here>`",
        inline=True)
    embed.add_field(
        ...
        inline=True)
    embed.set_footer(
        text="Coded by sgtbonsai in collaboration with Daniel Kravec")
    await ctx.send(embed=embed)
